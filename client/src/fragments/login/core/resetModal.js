...
import { ErrorText, isNotEmpty, PasswordValidator } from "@eachbase/utils";
...
export const ResetModal = ({ resetToken, reset, register }) => {
   ...
   const resetPassword = () => {
      const resetPassDataIsValid =
         isNotEmpty(inputs.newPassword) && isNotEmpty(inputs.confirmPassword);

      if (resetPassDataIsValid) {
         const passwords = {
         ...
         if (!isNotEmpty(inputs.newPassword)) {
            setError("New password is not field");
         } else if (!isNotEmpty(inputs.confirmPassword)) {
   ...
   return (
      <div className={classes.LoginModalWrapper}>
         <p>
            {register === true ? "Welcome Wellness Daisy " : "Reset Password"}
         </p>
         <span className={classes.LoginModalForgotText}>
            {register === true ? (
               <span>
                  You have been invited to join Wellness Daisy as a an admin.
                  Please create password for testuser@gmail.com
               </span>
            ) : (
               <span>
                  {" "}
                  ...
                  <br />
                  bv Use at least 8 characters, 1 upper case and 1 digit.
               </span>
            )}
         </span>
