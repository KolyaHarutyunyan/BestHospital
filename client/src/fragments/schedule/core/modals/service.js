...
import {CreateChancel, Loader, SelectInput, ValidationInput} from "@eachbase/components";
import {
    ErrorText,
    FindError,
    FindLoad,
...
export const Service = ({handleOpenClose, date, clientList, staffList, places, allPaycodes, modalDate, day, createModalDate,}) => {
    ...
    const [inputs, setInputs] = useState(
        modalDate
            ? {
                ...
                authorizedService: modalDate.authorizedService ? modalDate.authorizedService._id : '',
    ...
    useEffect(() => {
        if (modalDate) {
            setEditLoader(true)
            axios.get(`/authorization/client/${modalDate.client._id}`, {auth: true})
                .then((res) =>
                    res.data.length
                        ? axios.get(`/authorizationservice/authorization/${res.data[res.data.length - 1].id}`, {auth: true})
                            .then((date) => {
