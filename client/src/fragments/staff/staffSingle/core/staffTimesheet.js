...
import { DrawerContext, FindLoad, Images } from "@eachbase/utils";
         ...
         >
            <TableCell>
               <SlicedText
                  size={30}
                  type={"name"}
                  data={item?.payCode?.payCodeTypeId?.name}
               />
            </TableCell>
         ...
         <TableBodyComponent key={index}>
            <TableCell>
               <SlicedText
                  ...
                  data={
                     item.description
                        ? item?.payCode?.payCodeTypeId?.type
                        : item
                        ? item.rateType
                        : ""
                  }
                  ...
                  <>
                     <div className={classes.bcbaHeader}>
                        ...
                        <div className={classes.dateEdite}>
                           <p>
                              {timesheetById
                                 ? `${moment(timesheetById.startDate).format(
                                      "DD/MM/YYYY"
                                   )} - ${
                                      timesheetById.endDate
                                         ? moment(timesheetById.endDate).format(
                                              "DD/MM/YYYY"
                                           )
                                         : "Present"
                                   }`
                           ...
                           <img
                              src={Images.edit}
                              alt="edit"
                              onClick={() => handleEditClose(timesheetById)}
                           />
                        </div>
                     </div>
                     ...
                     <div style={{ height: "250px" }}>
                        {timesheetById && timesheetById.overtimes ? (
                           <Notes
                              restHeight="560px"
                              data={
                                 timesheetById.overtimes.length > 0
