...
function* getPostingById(action) {
   ...
   yield put(httpRequestsOnLoadActions.appendLoading(action.type));
   try {
      const res = yield call(postingService.getPostingByIdService, action.payload.id);
      yield put({
...
function* editPosting(action) {
   ...
   yield put(httpRequestsOnLoadActions.appendLoading(action.type));
   try {
      yield call(postingService.editPostingService, action.payload.id, action.payload.body);
      yield put({
         type: GET_POSTINGS,
      });
      yield put(httpRequestsOnLoadActions.removeLoading(action.type));
