...
import {CREATE_MILEAGE, DELETE_MILEAGE, EDIT_MILEAGE, GET_MILEAGES, GET_MILEAGES_SUCCESS,
} from "./milage.types";
...
function* deleteMileages(action) {
  ...
  try {
    const res = yield call( authService.deleteMileageService, action.payload.id );
    yield put({
      type:GET_MILEAGES
    })
